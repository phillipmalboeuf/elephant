...
export const load = (async ({ locals, url, params }) => {
  ...
  const all = await content.getEntries<TypeProjectSkeleton>({ content_type: "project", include: 1, select: ["fields.categories"], limit: 200 })
  ...
  const [pages, projects] = await Promise.all([
    content.getEntries<TypePageSkeleton>({ content_type: "page", include: 3, "fields.id": "projects" }),
    url.searchParams?.size && content.getEntries<TypeProjectSkeleton>({ content_type: "project", include: 2, ...filter ? { links_to_entry: categories[filter].sys } : {} }),
  ])
